...
int main()
{
	...
	cout << "Select the level of difficulty:" << endl << "1)Easy" << endl << "2)Normal" << endl << "3)Hard" << endl;//выбираем уровень сложности
	cin >> lvl;
	b = new string[200];
	ans = new string[200];
	switch (lvl){
	...
	while (cin1 >> a){//считываем с файла текст
		...
		break;
	case 2:
	while (cin2 >> a){//считываем с файла текст
		a += ' ';
		b[counts] = a;
		input += a;
		counts++;
	...
	break;
	case 3:
	while (cin3 >> a){//считываем с файла текст
		a += ' ';
		b[counts] = a;
		input += a;
		counts++;
	...
	time(&start1);//засекаем время
	for (i = 0; i < counts; ++i){//Идем по всем словам, которые мы считали
		SetConsoleCursorPosition(hout, point);//Устанавливаем курсор в начало
		for (int j = 0; j < i + 1; ++j){//Выводим слова, которые пользователю надо вводить
			cout << b[j];
		}
		if (i != 0){//здесь мы вычисляем на сколько надо сдвигать курсор
			point1.X += ans[i - 1].size();//а сдвигать его надо на размер предыдущего слова
			if (point1.X >= 80)
				point1.X -= 80, point1.Y++;
		}

		ramka();//рисуем рамку
		SetConsoleCursorPosition(hout, point1);//устанавливаем курсор на место для ввода

		while (1){//посимвольно читаем то, что вводит пользователь
			c = getch();//собственно читаем через getch()
			cout << c;//выводим то, что он написал
	...
	for (int i = 0; i < min(input.size(), output.size()); ++i){//в этом цикле мы идем до размера минимальной из строк
		if (input[i] != output[i]){//проверяем, ошибки
			anserr++;//считаем их количество
		}
	}
	anserr += modul(input.size() - output.size());//нам надо к ошибка добавить разницу размеров входной и выходной строки
	cout << "Your speed: " << float(output.size()) / ((float(end1) / 60)) << " Chars per minute!" << endl;//выводим скорость ввода

	cout << "Errors: " << anserr << endl;//количество ошибок
	result(anserr);//и результат
	getch();
	return 0;
}
